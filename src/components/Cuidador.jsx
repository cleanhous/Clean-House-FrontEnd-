...
const Cuidador = () => {
  ...
  // Estados para os cuidador
  const [cuidador, setCuidador] = useState([]);
  const [filteredCuidador, setFilteredCuidador] = useState([]);
  ...
  // Função para buscar cuidador
  const fetchCuidador = async () => {
    try {
      const response = await fetch("http://localhost:3000/cuidador");
      const data = await response.json();
      setCuidador(data);
      setFilteredCuidador(data); // Inicializa com todos os cuidador
    } catch (error) {
      console.error("Erro ao buscar cuidador:", error);
    }
  };

  // Hook para buscar os cuidador quando o componente monta
  useEffect(() => {
    fetchCuidador();
  }, []);

  // Função para aplicar os filtros e buscar cuidador disponíveis com base nas informações fornecidas
  const handleFiltrar = async () => {
    try {
      ...
      if (dataInicial && dataFinal) {
        ...
        // Se as datas não forem fornecidas, buscar todos os cuidador
        const response = await fetch("http://localhost:3000/cuidador");
        data = await response.json();
      }

      // Aplicar os filtros de nota e preço no frontend
      const filtered = data.filter((cuidador) => {
        let matches = true;

        // Filtrar por nota
        if (filtroNota) {
          matches = matches && parseInt(cuidador.nota) === parseInt(filtroNota);
        }

        // Filtrar por preço mínimo
        if (precoDe) {
          matches = matches && cuidador.preco >= parseFloat(precoDe);
        }

        // Filtrar por preço máximo
        if (precoAte) {
          matches = matches && cuidador.preco <= parseFloat(precoAte);
        }

        return matches;
      });

      // Atualiza a lista de cuidador exibidos com os filtros aplicados
      setFilteredCuidador(filtered);
  ...
  const handleCheckAvailability = (cuidador) => {
  ...
  const handleConfirmation = async () => {
    try {
      ...
      const response = await fetch("http://localhost:3000/contrato", {
        ...
        body: JSON.stringify({
          prestadorId: selectedCuidador.id,
    ...
    <div className="min-h-screen flex flex-col bg-sky-700">
      ...
      <div className="flex-1 p-8">
        ...
        <p className="text-center text-lg text-white mb-8">
          Oferecemos serviços de cuidador de alta qualidade, garantindo
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {filteredCuidador.length > 0 ? (
            filteredCuidador.map((cuidador) => (
              <div
                key={cuidador.id}
              ...
              >
                ...
                <p className="text-gray-600">{cuidador.titulo}</p>
                <p className="text-gray-600">{cuidador.descricao}</p>
                <p className="font-semibold text-sky-600">
                  {formatarPreco(cuidador.preco)}
                </p>
                ...
                <div className="flex items-center">
                  {Array.from({ length: 5 }, (_, index) => (
                    <svg
                      key={index}
                      className={`w-4 h-4 ${
                        index < Math.floor(cuidador.nota)
                          ? "text-yellow-500"
                          : "text-gray-400"
                      }`}
                      fill="currentColor"
                ...
                <div className="mt-4 flex justify-center">
                  <button
                    ...
                    onClick={() => handleCheckAvailability(cuidador)}
                  >
                    Contratar {cuidador.nome}
                  </button>
                </div>
              </div>
            ))
        ...
        <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center">
          <div className="bg-white p-6 rounded-lg shadow-lg w-96">
            <h2 className="text-xl font-bold text-sky-700 mb-4">
              Disponibilidade de {selectedCuidador.nome}
            </h2>
      ...
      {showConfirmationPopup && (
        <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center">
          <div className="bg-white p-6 rounded-lg shadow-lg w-96">
            ...
            <p className="mb-4">
              A contratação do cuidador {selectedCuidador.nome} foi
